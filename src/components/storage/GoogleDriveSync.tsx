...
import { GoogleDriveIcon } from "./ProviderIcons";
...
import { oauthConfig, isGoogleDriveConfigured, getGoogleClientId, isNativePlatform } from "@/config/oauth";
...
export const GoogleDriveSync = ({ onConfigChange, masterKey, onRequirePassword, isPrimary }: GoogleDriveSyncProps) => {
  // Initialize isConnected from ConnectionStateManager only - single source of truth
  const [isConnected, setIsConnected] = useState(() => {
    return connectionStateManager.isConnected('Google Drive');
  });
  const [isConnecting, setIsConnecting] = useState(false);
  const [service] = useState(() => new GoogleDriveService());
  ...
  // Guards to prevent double OAuth processing
  ...
  useEffect(() => {
    const handleManagerUpdate = () => {
      const managerConnected = connectionStateManager.isConnected('Google Drive');
      const primary = connectionStateManager.isPrimaryProvider('Google Drive');
      ...
      if (!managerConnected && currentlyConnected) {
        if (import.meta.env.DEV) console.log('🔔 [Google] External disconnect detected via manager');
  ...
  useEffect(() => {
    const handlePasswordCancelled = () => {
      if (import.meta.env.DEV) console.log('🔄 [Google] Password dialog cancelled - clearing pending state');
  ...
  useEffect(() => {
    if (masterKey && pendingConnection && !isConnecting) {
      console.log('✅ [Google] Master key now available, auto-retrying connection');
      setPendingConnection(false);
  ...
  useEffect(() => {
    const handleEncryptionInitialized = (event: CustomEvent) => {
      if (pendingConnection && event.detail.hasMasterKey && !isConnecting) {
        console.log('🔑 [Google] Encryption initialized - processing pending connection');
  ...
  useEffect(() => {
    const loadCredentials = async () => {
      ...
      if (connectionStateManager.isConnected('Google Drive')) {
        if (import.meta.env.DEV) console.log('🔗 [Google] Already connected via ConnectionStateManager, skipping credential load');
        setIsConnected(true);
      ...
      if (connectionStateManager.isExplicitlyDisabled('Google Drive')) {
        if (import.meta.env.DEV) console.log('🚫 [Google] Explicitly disabled, skipping credential load');
        return;
      }
      ...
      let creds: GoogleDriveCredentials | null = null;
      
      // Try E2E mode first if masterKey available
      if (masterKey && isE2EEnabled()) {
        creds = await CloudCredentialStorage.loadCredentials<GoogleDriveCredentials>(
          'google-drive',
          masterKey
        );
      }
      ...
      if (!creds && !isE2EEnabled()) {
        creds = SimpleModeCredentialStorage.loadGoogleDriveCredentials();
      }
      ...
      if (creds) {
        ...
        connectionStateManager.registerProvider('Google Drive', service);
  ...
  useEffect(() => {
    const handleEncryptionInitialized = async (event: CustomEvent) => {
      if (pendingOAuthParams && event.detail.hasMasterKey) {
        if (import.meta.env.DEV) console.log('🔑 [Google] Encryption initialized event - processing pending OAuth params');
        ...
        if (key && pendingOAuthParams) {
          ...
          try {
            ...
            console.error('❌ [Google] Failed to exchange pending OAuth tokens:', error);
            setPendingOAuthParams(null);
            ...
            if (!isSigningOut()) {
              toast({
                ...
                description: t('storage.googleAuthFailed'),
                variant: "destructive",
              });
            }
          } finally {
  ...
  useEffect(() => {
    const handleEncryptionPendingOAuth = async () => {
      ...
      if (!pendingOAuthParams) {
        if (import.meta.env.DEV) console.log('🔍 [Google] encryption-pending-oauth received but no pending params');
        return;
      }
      
      // Check if already processing
      if (processingOAuthRef.current) {
        if (import.meta.env.DEV) console.log('🔍 [Google] encryption-pending-oauth - already processing');
        return;
      }
      ...
      if (isOAuthCodeProcessed(pendingOAuthParams.code) || processedCodesRef.current.has(pendingOAuthParams.code)) {
        if (import.meta.env.DEV) console.log('⏭️ [Google] encryption-pending-oauth - code already processed');
        setPendingOAuthParams(null);
        return;
      }
      
      if (import.meta.env.DEV) {
        console.log('🔑 [Google] encryption-pending-oauth received - processing OAuth now');
      ...
      try {
        ...
        console.error('❌ [Google] Failed to process pending OAuth (deferred key):', error);
        setPendingOAuthParams(null);
        sessionStorage.removeItem('pending-oauth-provider');
        if (!isSigningOut()) {
          toast({
  ...
  useEffect(() => {
    const processPendingWithNewKey = async () => {
      ...
      // Check if code already processed
      if (isOAuthCodeProcessed(pendingOAuthParams.code) || processedCodesRef.current.has(pendingOAuthParams.code)) {
        if (import.meta.env.DEV) console.log('⏭️ [Google] Master key available but code already processed');
        setPendingOAuthParams(null);
        return;
      }
      
      if (import.meta.env.DEV) console.log('🔑 [Google] Master key now available via props - processing pending OAuth');
      ...
      try {
        ...
        console.error('❌ [Google] Token exchange failed (masterKey prop change):', error);
        setPendingOAuthParams(null);
        if (!isSigningOut()) {
          toast({
            title: t('storage.connectionFailed'),
  ...
  // Handle OAuth redirect from sessionStorage (native Android via StorageOAuthCallback)
  // and from URL params (web OAuth redirect)
  useEffect(() => {
    // Check sessionStorage first (set by StorageOAuthCallback on native)
    const storedCode = sessionStorage.getItem('storage-oauth-code');
    const storedState = sessionStorage.getItem('storage-oauth-state');
    const storedProvider = sessionStorage.getItem('storage-oauth-provider');
    
    let code = storedCode;
    let state = storedState;
    
    // Clear stored params immediately to prevent re-processing
    if (storedCode) {
      ...
      console.log('📥 [Google] Found OAuth params in sessionStorage (native callback)');
    }
    
    // If not from sessionStorage, check URL params (web flow)
    if (!code) {
    ...
    // EARLY EXIT: If no OAuth params and no pending params, nothing to do
    if (!code && !error && !pendingOAuthParams) {
      return;
    }
    
    // For sessionStorage params, verify provider
    if (storedCode && storedProvider && storedProvider !== 'google-drive') {
      console.log(`⏭️ [Google] Skipping - OAuth callback is for ${storedProvider}`);
      return;
    }
    
    // Guard against multiple simultaneous processing
    if (processingOAuthRef.current) {
      if (import.meta.env.DEV) console.log('🔍 [Google] handleOAuthRedirect - already processing, skipping');
      return;
    }
    ...
    const handleOAuthRedirect = async () => {
      ...
      // These were stored specifically because they WEREN'T processed yet
      if (pendingOAuthParams && masterKey) {
        if (import.meta.env.DEV) console.log('🔑 [Google] Processing pending OAuth params now that master key is available');
        processingOAuthRef.current = true;
        // NOW mark as processed since we're about to exchange tokens
        processedCodesRef.current.add(pendingOAuthParams.code);
        markOAuthCodeProcessed(pendingOAuthParams.code);
        setIsConnecting(true);
      ...
      // Handle OAuth errors
      if (error) {
        // Only handle if this is a Google-related error
        const provider = state ? getProviderFromState(state) : null;
        if (provider && provider !== 'google-drive') {
          console.log(`⏭️ [Google] Skipping error handling - belongs to ${provider}`);
          return;
        }
        cleanOAuthUrl(); // Clean URL on error
        const safeMessage = errorDesc?.includes('access_denied') 
      ...
      if (code && state) {
        // Check if this callback is for Google Drive
        const provider = getProviderFromState(state);
        if (provider !== 'google-drive') {
          console.log(`⏭️ [Google] Skipping OAuth callback - belongs to ${provider}`);
          return;
        }
        ...
        if (isOAuthCodeProcessed(code) || processedCodesRef.current.has(code)) {
          console.log('⏭️ [Google] Code already processed (persisted), skipping');
          return;
        }
        
        console.log('🔐 [Google] Processing OAuth callback for Google Drive');
        
        // Clean URL immediately to prevent re-triggering
        cleanOAuthUrl();
        
        // Mark as processing but DON'T persist yet - only after successful token exchange
        processingOAuthRef.current = true;
        
        const encryptionMode = getEncryptionMode();
        
        // In E2E mode, require master key - store params for later processing
        if (encryptionMode === 'e2e' && !masterKey) {
          ...
          if (storageServiceV2.isPendingOAuth || isOnboardingOAuthReturn) {
            if (import.meta.env.DEV) console.log('🔑 [Google] Detected onboarding OAuth return - proceeding with deferred credentials');
            processingOAuthRef.current = true;
            ...
            try {
              ...
              console.error('❌ [Google] Token exchange failed (pending-oauth):', error);
              if (!isSigningOut()) {
                toast({
                  title: t('storage.connectionFailed'),
                  description: t('storage.googleAuthFailed'),
          ...
          if (storageServiceV2.isInitializationInProgress) {
            console.log('🔄 [Google] Initialization in progress - waiting instead of requesting password');
          ...
          if (existingKey) {
            if (import.meta.env.DEV) console.log('🔑 [Google] Master key exists in storageService - processing immediately');
            // Process with existing key - it will propagate via listeners
            processedCodesRef.current.add(code);
            markOAuthCodeProcessed(code);
            setIsConnecting(true);
            try {
          ...
          console.log('🔒 [Google] OAuth callback in E2E mode but master key not available - storing params');
          setPendingOAuthParams({ code, state });
          processingOAuthRef.current = false;
          ...
          sessionStorage.setItem('pending-oauth-provider', 'google-drive');
          ...
          import('@/services/encryptionStateManager').then(({ encryptionStateManager }) => {
            encryptionStateManager.requestPasswordIfNeeded('GoogleDriveSync-OAuth');
          });
          onRequirePassword?.(() => {});
          return;
        }
        ...
        try {
          ...
          if (!isSigningOut()) {
            const safeMessage = error instanceof Error && error.message?.includes('OAuth')
              ? t('providers.oauth.googleDriveAuthFailed')
              : t('providers.google-drive.connectionFailed');
            toast({
              title: t('storage.connectionFailed'),
              description: safeMessage,
              variant: "destructive",
  ...
  const exchangeCodeForTokens = async (code: string, state: string, keyOverride?: CryptoKey | null): Promise<void> => {
    if (!isGoogleDriveConfigured()) {
      throw new Error('Google Drive OAuth not configured - missing client ID');
    }

    // Retrieve and validate PKCE verifier + state
    const codeVerifier = retrievePKCEVerifier('google-drive', state);
    if (!codeVerifier) {
    ...
    console.log('🔐 [Google] Token exchange via edge function, redirect URI:', redirectUri);
    
    // Use edge function to exchange code (keeps client_secret server-side)
    const supabaseUrl = SUPABASE_CONFIG.url;
    
    // Get auth token for edge function authentication
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.access_token) {
      throw new Error('Authentication required for OAuth token exchange');
    }
    ...
    // Validate token response structure with scope validation
    const expectedScopes = ['https://www.googleapis.com/auth/drive.appdata'];
    const validatedTokens = validateTokenResponse(data, expectedScopes);
    ...
    if (isE2EEnabled() && effectiveKey) {
      console.log('🔐 [Google] Saving credentials with E2E encryption');
      await CloudCredentialStorage.saveCredentials(creds, effectiveKey);
    } else {
      console.log('📦 [Google] Saving credentials in Simple mode');
      SimpleModeCredentialStorage.saveGoogleDriveCredentials(creds);
    }
    ...
    if (wasOnboardingOAuth) {
      console.log('📣 [Google] Dispatching onboarding-oauth-complete event');
    ...
    const isPrimary = connectionStateManager.isPrimaryProvider('Google Drive');
    
    // Force entry reload in Index.tsx after OAuth connection
    console.log('📥 [Google] Dispatching trigger-sync to reload entries');
    ...
    toast({
      title: t('storage.googleConnected'),
   ...
   * 2. Connect service to Google Drive (only needs access token)
  ...
  const exchangeCodeForTokensWithDeferredCredentials = async (code: string, state: string): Promise<void> => {
    ...
    const redirectUri = getOAuthRedirectUri('google-drive');
    console.log('🔐 [Google] (Deferred) Token exchange via edge function, redirect URI:', redirectUri);
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error('🔴 [Google] (Deferred) Token exchange error:', errorData);
      throw new Error(
        `OAuth token exchange failed: ${errorData.error_description || errorData.error || response.statusText}`
      );
    }
    ...
    console.log('🔐 [Google] (Deferred) Connecting service WITHOUT master key...');
    ...
    console.log('✅ [Google] (Deferred) Service connected, registering with manager...');
    connectionStateManager.registerProvider('Google Drive', service);
    storageServiceV2.enableSync();
    ...
    console.log('🔑 [Google] (Deferred) Calling onCloudProviderConnected to derive master key...');
    ...
    if (import.meta.env.DEV) console.log('🔐 [Google] (Deferred) Retrieved stored password:', storedPassword ? 'yes' : 'no');
    
    const result = await storageServiceV2.onCloudProviderConnected(storedPassword);
    
    if (result.requiresPassword) {
      console.log('🔐 [Google] (Deferred) Still needs password:', result.reason);
      onRequirePassword?.();
      return;
    }
    ...
    if (derivedKey) {
      console.log('🔐 [Google] (Deferred) Master key derived, saving encrypted credentials...');
      await CloudCredentialStorage.saveCredentials(creds, derivedKey);
    } else {
      console.warn('⚠️ [Google] (Deferred) No master key after onCloudProviderConnected - credentials not encrypted');
    }
    
    // CRITICAL: Check onboarding flag BEFORE clearing any session storage
    const wasOnboardingOAuth = sessionStorage.getItem('onboarding-oauth-in-progress');
    if (import.meta.env.DEV) console.log('🔍 [Google] (Deferred) onboarding-oauth-in-progress flag:', wasOnboardingOAuth);
    ...
    if (wasOnboardingOAuth) {
      console.log('📣 [Google] (Deferred) Dispatching onboarding-oauth-complete event');
      window.dispatchEvent(new CustomEvent('onboarding-oauth-complete'));
      sessionStorage.removeItem('onboarding-oauth-in-progress');
    }
    ...
    setIsConnected(true);
    onConfigChange?.(true, true);
    console.log('✅ [Google] (Deferred) Connection complete with E2E encryption');
    
    // Force entry reload in Index.tsx after deferred OAuth connection
    console.log('📥 [Google] Dispatching trigger-sync to reload entries');
    queueMicrotask(() => { window.dispatchEvent(new CustomEvent('trigger-sync')); });
  ...
  const handleConnect = async () => {
    // Get platform-appropriate client ID
    const clientId = getGoogleClientId();
    
    if (!isGoogleDriveConfigured()) {
      toast({
        title: t('storage.configError'),
        description: t('storage.googleClientIdMissing'),
        variant: "destructive",
      });
      return;
    }
    ...
    if (currentMode === 'e2e' && !masterKey) {
      console.log('🔐 [Google] Master key required for E2E mode - setting pending connection');
      // CRITICAL: Clear disabled status to allow reconnection after disconnect
      connectionStateManager.enableProvider('Google Drive');
    ...
    try {
      ...
      if (isNativePlatform()) {
        console.log('📱 [Google] Using native OAuth flow via authService');
      ...
      // PRIVACY: Use minimal scope (appdata only, not full drive access)
      ...
      const message = error instanceof Error ? error.message : t('providers.oauth.failedToStart');
      toast({
  ...
  /**
   * Handle OAuth on native Android platform via authService
   * Uses Capacitor Browser plugin with HTTPS App Links callback
   * Google requires HTTPS redirect URIs - custom schemes are rejected
   */
  const handleNativeOAuth = async (clientId: string) => {
    ...
    try {
      ...
      // Use HTTPS App Link - getOAuthRedirectUri handles native detection
      const redirectUri = getOAuthRedirectUri('google-drive');
      console.log('📱 [Google] Native OAuth with redirect URI:', redirectUri);
      ...
      if (result.success && result.tokens) {
        console.log('✅ [Google] Native OAuth successful');
        ...
        if (isE2EEnabled() && masterKey) {
          console.log('🔐 [Google] Saving credentials with E2E encryption (native)');
          await CloudCredentialStorage.saveCredentials(creds, masterKey);
        } else {
          console.log('📦 [Google] Saving credentials in Simple mode (native)');
          SimpleModeCredentialStorage.saveGoogleDriveCredentials(creds);
        }
        ...
        // Trigger onCloudProviderConnected for encryption key handling
        await storageServiceV2.onCloudProviderConnected();
        
        // Force entry reload in Index.tsx after native OAuth connection
        console.log('📥 [Google] Dispatching trigger-sync to reload entries (native)');
        queueMicrotask(() => { window.dispatchEvent(new CustomEvent('trigger-sync')); });
        
        toast({
          title: t('storage.googleConnected'),
          description: t('storage.readyForSync'),
        });
      } else {
      ...
      if (!isSigningOut()) {
        toast({
          title: t('storage.connectionFailed'),
          description: error instanceof Error ? error.message : t('storage.googleAuthFailed'),
          variant: "destructive",
        });
      }
    } finally {
  ...
  const handleDisconnect = async () => {
    ...
    if (import.meta.env.DEV) {
      console.log('🔌 Disconnecting Google Drive - clearing all credentials');
    }
    ...
    connectionStateManager.unregisterProvider('Google Drive');
    ...
    // STEP 5: PRIVACY: Revoke tokens on provider's side
    await service.disconnect();
    
    // STEP 6: Clear credentials from BOTH storage systems using force methods
    CloudCredentialStorage.forceRemoveCredentials('google-drive');   // E2E mode - force remove
    SimpleModeCredentialStorage.clearGoogleDriveCredentials();       // Simple mode
    
    // Verify credentials are actually cleared
    if (import.meta.env.DEV) {
      const e2eStillExists = CloudCredentialStorage.hasCredentials('google-drive');
      const simpleStillExists = SimpleModeCredentialStorage.hasGoogleDriveCredentials();
    ...
    toast({
      ...
      description: t('storage.googleDisconnected')
    });
  };

  // ConnectionStateManager now handles window bindings
    ...
    return (
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4">
          <GoogleDriveIcon className="w-5 h-5 text-blue-600" />
          <h3 className="text-lg font-semibold">{t('storage.googleDrive')}</h3>
        </div>
        <p className="text-muted-foreground mb-4">
          {t('storage.googleDriveDesc')}
        </p>
        
        {import.meta.env.DEV && !isGoogleDriveConfigured() && (
          <Alert className="mb-4">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription className="text-sm">
              <strong>{t('storage.devMode')}:</strong> {t('storage.googleDevInstructions')}
            </AlertDescription>
          </Alert>
        )}
        ...
        {getEncryptionMode() === 'simple' && (
          <Alert className="mb-4 border-amber-200 bg-amber-50 dark:bg-amber-950/20">
            ...
            <AlertDescription className="text-amber-800 dark:text-amber-400 text-xs">
              <strong>{t('storage.simpleMode')}:</strong> {t('storage.googleSimpleModeWarning')}
            </AlertDescription>
          </Alert>
        )}
        ...
        >
          ...
          {isConnecting ? t('storage.connecting') : t('storage.connectGoogleDrive')}
        </Button>
      </Card>
    );
  }
    ...
    <Card className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <GoogleDriveIcon className="w-5 h-5 text-blue-600" />
          <h3 className="text-lg font-semibold whitespace-nowrap">{t('storage.googleDrive')}</h3>

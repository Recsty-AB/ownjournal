      ...
      <Card className="p-4">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 space-y-2"><Label htmlFor="source">{t('storage.from')}</Label><Select value={selectedSource} onValueChange={setSelectedSource} disabled={isTransferring}><SelectTrigger id="source"><SelectValue placeholder={t('storage.selectSource')} /></SelectTrigger><SelectContent>{providers.map((p) => (<SelectItem key={p.name} value={p.name}>{p.name}</SelectItem>))}</SelectContent></Select></div>
            <ArrowRight className="w-4 h-4 mt-6 text-muted-foreground" />
            <div className="flex-1 space-y-2"><Label htmlFor="target">{t('storage.to')}</Label><Select value={selectedTarget} onValueChange={setSelectedTarget} disabled={isTransferring}><SelectTrigger id="target"><SelectValue placeholder={t('storage.selectTarget')} /></SelectTrigger><SelectContent>{providers.filter((p) => p.name !== selectedSource).map((p) => (<SelectItem key={p.name} value={p.name}>{p.name}</SelectItem>))}</SelectContent></Select></div>
          </div>

-- Fix: AI access gate must check current_period_end, not just is_pro.
--
-- Previously, get_ai_usage_summary returned the raw subscriptions.is_pro
-- column. If a RevenueCat / Stripe expiration webhook is missed (which
-- happens in Apple sandbox routinely and in production occasionally), the
-- row stays at is_pro=true with current_period_end in the past, and
-- ai-analyze keeps granting AI usage to a user whose subscription has
-- ended. Mirror the client-side isEntitlementActive() rule on the server
-- so the read is correct regardless of webhook delivery.
...
AS $$
  ...
  WHERE s.user_id = p_user_id
    AND s.user_id = auth.uid()  -- CRITICAL: Verify caller owns this data
  GROUP BY s.is_pro, s.current_period_end
$$;
